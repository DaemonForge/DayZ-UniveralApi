...
class UScriptExec
{
    ...
    {
        FileHandle file;
        MakeDirectory("$profile:UF/Scripts/"); // create directory for logging and execution purposes
		string filename = string.Format("script_%1", GetDateTime());
        string script_path = string.Format("$profile:UF/Scripts/%1.c", filename);
		int i = 0;
		while (FileExist(script_path) && i < 32){
			filename = string.Format("script_%1_%2_%3", GetDateTime(), i++, Math.RandomInt(10000,99999));
        	script_path = string.Format("$profile:UF/Scripts/%1.c", filename);
		}
        ...
        if (!module)
        {
            Error2("[UF][FUNC]","Invalid syntax! Review your script. " + code);
            return;
        }
	...
	static Class Spawn(string className){
		if (className.ToType()){
			...
			Error2("[UF][SPAWN]", "'" + className + "' wasn't a valid classname couldn't spawn" );
		}
		return NULL;
	}

    // uses UFScriptBase
	static bool SpawnScriptBase(string className, out UFScriptBase instance){
		if (className.ToType()){
			instance = UFScriptBase.Cast(className.ToType().Spawn());
			instance.Init();
			return true;
		}
		return false;
