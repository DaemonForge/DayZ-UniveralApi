class UJSONHandler<Class T>
{
	...
	static bool FromString(string stringData, out T data)
	{
		...
		if (stringData == "" || !m_Serializer.ReadFromString(data, stringData, error)) {
			Print(stringData);
			Error2("[UF] FROM STRING", "Error Creating Data from Json : " + error);
			return false;
		}
		return true;
	}
	...
	static string ToString(T data)
	{
		...
		if (!m_Serializer.WriteToString(data, false, stringData)) {
			Print(stringData);
			Error2("[UF] TO STRING", "Error Creating JSON from Data");
			return "{}";
		}
	...
	static bool GetString(T data, out string stringData)
	{
		...
		if (!m_Serializer.WriteToString(data, false, stringData)) {
			Print(stringData);
			Error2("[UF] GET STRING", "Error Creating JSON from Data");
			return false;
		}
