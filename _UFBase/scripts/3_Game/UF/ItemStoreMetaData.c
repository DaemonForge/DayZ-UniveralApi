class UAmmoData extends Managed{
	protected int m_cartIndex;
	protected float m_dmg;
	protected string m_cartTypeName;
	
	void UAmmoData(int cartIndex, float dmg, string cartTypeName){
...
class UMetaData extends Managed{
	...
	void UMetaData(string var, string data){
		Var = var;
		Data = data;
	}
...
class UZoneData extends Managed{
	string m_Zone;
	float m_Health;
	
	void UZoneData(string zone, float health){
		m_Zone = zone;
		m_Health = health;
	}
