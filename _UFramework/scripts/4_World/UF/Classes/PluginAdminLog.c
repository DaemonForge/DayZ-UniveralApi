modded class PluginAdminLog extends PluginBase
{
	...
	{
		if (UFConfig().EnableBuiltinLogging != 2){
			super.PlayerKilled( player, source );
		}
		
		if ( UFConfig().EnableBuiltinLogging != 0 && player && source && player.GetIdentity() )
		{
			ULogKilled logobj;
			if ( player == source ){ // deaths not caused by another object (starvation, dehydration)
				logobj = new ULogKilled(player.GetIdentity().GetId(), player.GetPosition());
			} else {
				logobj = new ULogKilled(player.GetIdentity().GetId(), player.GetPosition(), source.GetType(), source.GetPosition() );
			}
	...
	*/
	...
	override void OnPlacementComplete( Man player, ItemBase item ) // ItemBase.c
	{
		...
		if ( UFConfig().EnableBuiltinLogging != 0)
		{
	...
	override void OnContinouousAction( ActionData action_data )	// ActionContinouousBase.c
	{
		...
		if ( UFConfig().EnableBuiltinLogging != 0 && action_data)
		{				
	...
	override void Suicide( PlayerBase player )  // EmoteManager.c 
	{
		...
		if ( UFConfig().EnableBuiltinLogging != 0 && player && player.GetIdentity() )
		{
			ULogKilled logobj = new ULogKilled(player.GetIdentity().GetId(), player.GetPosition(), "Suicide");
	...
	override void BleedingOut( PlayerBase player )  // Bleeding.c
	{
		...
		if ( UFConfig().EnableBuiltinLogging != 0 &&  player && player.GetIdentity() )
		{
			ULogKilled logobj = new ULogKilled(player.GetIdentity().GetId(), player.GetPosition(), "BleedingOut");
	...
	override void PlayerList() {
		...
		if (UFConfig().EnableBuiltinLogging != 0 ){
			thread DoUFPlayerListLog(); //To stop any extra server lag
		}
	}
		...
		if ( m_PlayerArray.Count() != 0 ) {	
			for (int i = 0; i < theManList.Count(); i++ ) {
				...
				if (thePlayer && thePlayer.GetIdentity()) { 
					...
					thePlayerList.Insert(new ULogPlayerPos(thePlayer.GetIdentity().GetId(), thePlayer.GetPosition(), MovementSpeed, thePlayer.IsInTransport()) );
				}					
			}
		}
