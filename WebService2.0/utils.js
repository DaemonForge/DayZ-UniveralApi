...
let {createHash, verify} = require('crypto');
const RateLimit = require('express-rate-limit');
let {generate} = require('selfsigned');
...
async function IncermentAPICount(clientId){
  ...
  try {
    ...
    if (day.length < 2) 
        day = '0' + day;
    let query = { ClientId: clientId, Month: `${date.getFullYear()}-${month}`, Day: `${day}` };
    let data = {"$inc": { Count: 1 } };
...
async function CheckRecentVersion(){
  ...
  async function GenerateCerts(){
    let attrs = { name: 'localhost', value: 'localhost.localhost' };
     return generate(attrs, { days: 3650 });
  }