...
async function runStatusCheck(req, res, auth) {
    ...
    try {
        ...
        const updateDocValue = {
            Mod: "UniversalApiStatus",
            Description: "This Object Exsits as a test when ever the status url is called to make sure the database is writeable",
            TestVar: TestValue
        }
        const updateDoc = {
            $set: updateDocValue,
        ...
        if (result.modifiedCount >= 1 || result.upsertedCount >= 1) {
            res.json({
                ...
                Discord: global.DISCORDSTATUS,
                Translate: TranslatesEnabled,
                Wit: WitsEnabled,
                QnA: QnAEnabled,
                LUIS: LUISEnabled
            });
