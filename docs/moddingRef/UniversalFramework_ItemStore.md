...
### Custom Data Storage
DayZ entity saving only handles vanilla variables. If your mod adds custom variables to an item (e.g., a "KillCount" on a gun), you **MUST** override `OnUFSave` and `OnUFLoad` to persist them.

```enforce
modded class MyGun extends Rifle_Base {
    int m_KillCount;

    override void OnUFSave(UEntityStore data) {
        super.OnUFSave(data);
        data.Write("killCount", m_KillCount);
    }
    
    override void OnUFLoad(UEntityStore data) {
        super.OnUFLoad(data);
        data.Read("killCount", m_KillCount);
    }
}
```

### Hierarchy Handling
The `SaveEntity(entity, true)` call is recursive. It saves the gun, the mag, the ammo in the mag, the optic, and the battery in the optic. You don't need to manually iterate children unless you are saving a detached list (like inventory contents).

## Common Use Cases

### Virtual Garage
Store vehicles in the cloud to save server FPS.
1. Player parks car.
2. `store.SaveEntity(car)`.
3. `car.Delete()`.
4. Save JSON to DB.
5. Retrieve JSON and `store.CreateAtPos()` to respawn it later.

### Loadout System
Allow players to save "Presets" of gear.
*   Save the player's inventory as an array of `UEntityStore` objects.
*   When loading, clear current inventory and recreate items from the array.

## Tags
`item-store`, `serialization`, `persistence`, `virtual-garage`, `loadouts`, `custom-data`, `UEntityStore`, `inventory`, `cargo`, `attachments`, `how-to`, `doc-usage`, `modder`
