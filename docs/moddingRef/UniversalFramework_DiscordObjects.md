...
### Object Ownership
Always use `autoptr` when creating complex objects like `UDiscordEmbed` or arrays of fields. This ensures memory is managed correctly and prevents leaks.

### Color Conversions
Discord uses integer colors. A common trick is to use Hex constants (e.g. `0xFF0000` for red).

## Tags
`discord`, `objects`, `embeds`, `messages`, `api`, `structure`, `data-classes`, `reference`, `how-to`, `doc-usage`, `modder`
