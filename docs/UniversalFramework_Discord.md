...
1. **Always handle UF_NOTSETUP** - many players won't have Discord linked
2. **Use embeds** for rich, formatted messages
3. **Rate limit Discord operations** - Discord has API limits
4. **Cache Discord user data** - don't query on every action
5. **Verify bot permissions** - ensure bot can perform required actions
6. **Handle voice channel errors** - user may not be in voice
