...
if (cluster.isMaster) {
    ...
    cluster.on('message', (worker, msg) => {
        if (msg.type === 'getNumbers') {
            ...
            // Reply to the worker with the numbers allocated.
    ...
    getRandom = async function(req, res) {
        ...
        try {
            ...
            logger.info("Random numbers requested from worker", { count });
...
async function getRandom(req, res){
        ...
        try {
            ...
            logger.info("Random numbers requested from master", { count, randomNubmersRemaing: randomNumbers.length });
            return res.status(200).json({Status: "Success", Error: "", Numbers: numbers });
        } catch (e) {
            logger.error('Error generating random numbers', { 
                error: e.message,
