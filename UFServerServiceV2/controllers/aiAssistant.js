...
async function createAssistant(req, res){
  try {
    ...
    logger.info("Assistant created", { AssistantId });
    return res.status(201).json({ Status: "Success", AssistantId: result.AssistantId });
  } catch (err) {
    logger.error("Error creating assistant: " + err.message, { stack: err.stack });
...
async function registerExistingAssistant(req, res){
  try {
    ...
    logger.info("Assistant registered", { AssistantId });
    return res.status(200).json({ Status: "Success", AssistantId: result.AssistantId });
  } catch (err) {
    logger.error("Error registering assistant: " + err.message, { stack: err.stack });
...
async function getAssistants(req, res){
  try {
    ...
    logger.error("Error listing assistants: " + err.message, { stack: err.stack });
...
async function updateAssistant(req, res){
  try {
    ...
    logger.info("Assistant updated", { AssistantId });
    return res.status(200).json({ Status: "Success", Message: "Assistant updated successfully" });
  } catch (err) {
    logger.error("Error updating assistant: " + err.message, { stack: err.stack });
...
async function deleteAssistant(req, res){
  try {
    ...
    logger.error("Error deleting assistant: " + err.message, { stack: err.stack });
...
async function createThread(req, res){
  try {
    ...
    logger.info("Thread created", { ThreadId: openaiThread.id });
    return res.status(201).json({ Status: "Success", ThreadId: openaiThread.id });
  } catch (err) {
    logger.error("Error creating thread: " + err.message, { stack: err.stack });
...
async function getThreadHistory(req, res){
  try {
    ...
    logger.error("Error retrieving thread history: " + err.message, { stack: err.stack });
...
async function sendMessageInThread(req, res){
  try {
    ...
    if (GUID !== undefined && GUID !== null && GUID !== "" && (thread.GUID !== GUID) && !isServer) {
      logger.warn("Unauthorized user attempted to send message in thread", { ThreadId, GUID });
    ...
    // Asynchronously process the assistant's response.
    let instructions = "";
    ...
    try {
      ...
      const run = await openai.beta.threads.runs.createAndPoll(
        ThreadId,
        { 
          assistant_id: assistant.AssistantApiId || "", // AssistantId may be passed in URL if needed,
      ...
      if (run.status === 'completed') {
        ...
        await aiAssistantModel.updateMessageStatus(ThreadId, MessageId, "Success", messages.data[0].content[0].text.value || "");
      } else {
        logger.error("Error sending message in thread: " + run.status, { stack: run });
      ...
      logger.error("Error sending message in thread: " + err.message, { stack: err.stack });
    }
    // Update the assistant placeholder in our DB with the completed response.
  } catch (err) {
    logger.error("Error sending message in thread: " + err.message, { stack: err.stack });
...
async function checkMessageStatus(req, res){
  try {
    ...
    logger.error("Error checking message status: " + err.message, { stack: err.stack });
...
async function runSummarizeChat(req, res) {
    try {
        const { ThreadId } = req.params;
        logger.info("Received request to summarize thread", { ThreadId });
        if (!ThreadId) {
            logger.warn("ThreadId missing in summarize request");
            return res.status(400).json({ Status: "Error", SummaryId: "", Summary: "", Error: "ThreadId is required" });
        }
        const thread = await aiAssistantModel.getThread(ThreadId);
        if (!thread) {
            logger.warn("Thread not found for summarization", { ThreadId });
            return res.status(200).json({ Status: "NotFound", SummaryId: "", Summary: "", Error: "Thread not found" });
        }
        // Check if a summary already exists on the thread record.
        ...
        if (thread.Summary !== undefined && thread.Summary !== "") { 
          console.log(thread);
            ...
            if (timeDif <= 1) {
                logger.info("Thread summary exists and is up-to-date", { ThreadId });
        ...
        logger.info("Created summary record", { ThreadId, SummaryId });
        res.status(202).json({ Status: "Pending", SummaryId, Summary: "", Error: "" });

        // Process the summary asynchronously.
        (async () => {
            try {
                logger.info("Starting asynchronous summary generation", { ThreadId, SummaryId });
                let conversationText = thread.SystemMessage ? thread.SystemMessage + "\n" : "";
                thread.messages.forEach(msg => {
                    conversationText += `${msg.role}: ${msg.content}\n`;
                });
                logger.info("Built conversation text for summarization", { ThreadId, SummaryId });
                ...
                logger.info("AI generated summary", { ThreadId, SummaryId, summaryText });
                ...
                logger.info("Updated summary record with success status", { ThreadId, SummaryId });
                await aiAssistantModel.saveChatSummary(ThreadId, summaryText);
                logger.info("Saved thread summary to storage", { ThreadId, SummaryId });
            } catch (err) {
                logger.error("Error during asynchronous summary generation", { ThreadId, SummaryId, error: err.message });
